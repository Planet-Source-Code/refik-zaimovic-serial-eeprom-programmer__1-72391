...
Private Sub DrawButton()
    ...
    Select Case SNorm
        Case 0: US = EDGE_RAISED
        Case 1: US = EDGE_SUNKEN
        Case 2: US = EDGE_BUMP
        Case 3: US = EDGE_ETCHED
        Case 4: US = EDGE_RFLAT
        Case 5: US = EDGE_SFLAT
    End Select
    
    Select Case SOver
        Case 0: OS = EDGE_RAISED
        Case 1: OS = EDGE_SUNKEN
        Case 2: OS = EDGE_BUMP
        Case 3: OS = EDGE_ETCHED
        Case 4: OS = EDGE_FLAT
        Case 5: OS = EDGE_SFLAT
    End Select
    
    Select Case SDown
        Case 0: DS = EDGE_RAISED
        Case 1: DS = EDGE_SUNKEN
        Case 2: DS = EDGE_BUMP
        Case 3: DS = EDGE_ETCHED
        Case 4: DS = EDGE_FLAT
        Case 5: DS = EDGE_SFLAT
    End Select
    ...
    If UpPic = False Then
        If mOrient = 0 Then
            SetBound Picture1, UserControl.ScaleWidth, 17
        Else
            SetBound Picture1, 17, UserControl.ScaleHeight
        End If
        AlignControls
        SetRect hRec1, 0, 0, Picture1.ScaleWidth, Picture1.ScaleHeight
        If UpBtn = True And CheckMouseOver(Picture1.hWnd) = True Then
            DrawArrow Picture1, True, True
            DrawEdge Picture1.hdc, hRec1, DS, BF_RECT
        Else
            DrawArrow Picture1, False, True
            If CheckMouseOver(Picture1.hWnd) = True And SDnBtn = False And SUpBtn = False And TmBtn = False And DnBtn = False Then
                DrawEdge Picture1.hdc, hRec1, OS, BF_RECT
                Tracking1.Enabled = True
            Else
                DrawEdge Picture1.hdc, hRec1, US, BF_RECT
            End If
        End If
    Else
        If UpBtn = True And CheckMouseOver(Picture1.hWnd) = True Then
    ...
    If TbPic = False Then
        If mOrient = 0 Then
            SetBound Picture2, UserControl.ScaleWidth, 17
        Else
            SetBound Picture2, 17, UserControl.ScaleHeight
        End If
        AlignControls
        SetRect hRec2, 0, 0, Picture2.ScaleWidth, Picture2.ScaleHeight
        If TmBtn = True And CheckMouseOver(Picture2.hWnd) = True Then
            DrawEdge Picture2.hdc, hRec2, DS, BF_RECT
        Else
            If CheckMouseOver(Picture2.hWnd) = True And SDnBtn = False And SUpBtn = False And UpBtn = False And DnBtn = False Then
                DrawEdge Picture2.hdc, hRec2, OS, BF_RECT
                Tracking2.Enabled = True
            Else
                DrawEdge Picture2.hdc, hRec2, US, BF_RECT
            End If
        End If
    Else
        If TmBtn = True And CheckMouseOver(Picture2.hWnd) = True Then
    ...
    If DnPic = False Then
        If mOrient = 0 Then
            SetBound Picture3, UserControl.ScaleWidth, 17
        Else
            SetBound Picture3, 17, UserControl.ScaleHeight
        End If
        AlignControls
        SetRect hRec3, 0, 0, Picture3.ScaleWidth, Picture3.ScaleHeight
        If DnBtn = True And CheckMouseOver(Picture3.hWnd) = True Then
            DrawArrow Picture3, True, False
            DrawEdge Picture3.hdc, hRec3, DS, BF_RECT
        Else
            DrawArrow Picture3, False, False
            If CheckMouseOver(Picture3.hWnd) = True And SDnBtn = False And SUpBtn = False And TmBtn = False And UpBtn = False Then
                DrawEdge Picture3.hdc, hRec3, OS, BF_RECT
                Tracking3.Enabled = True
            Else
                DrawEdge Picture3.hdc, hRec3, US, BF_RECT
            End If
        End If
    Else
        If DnBtn = True And CheckMouseOver(Picture3.hWnd) = True Then
    ...
    End If
        ...
        If CurTemp < 0 Then
            If (TrueVal + CurTemp) > 0 Then
                TrueVal = TrueVal + (CurTemp * tHandle)
                RaiseEvent Scroll
            Else
                TrueVal = 0
                RaiseEvent Scroll
